...
- Preserve the user's supplied Chinese source text verbatim; UI structure may expose it as chapters and nodes but must not rewrite or summarize the original reading text.
- The selected visual direction is the dark mobile mind-map player: midnight navy canvas, mint branches, amber active node, and a persistent bottom listening controller.
- Current content source is the user-provided “面试话术” mind map. Its 14 top-level branches and nested labels must remain verbatim and in source order.
- Under “项目”, child order is fixed as 2.1 星辰维度, 2.2 鸿泉, 2.3 招行. Each child owns its exact supplied reading text.
- Parse every supplied indentation level into a collapsible tree; never flatten nested mind-map content. Preserve emphasis with yellow (explicit/bold), orange (key result), red (risk/error), and purple (AI/technical) treatments.
